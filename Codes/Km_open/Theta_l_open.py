...
print('The sign of spacial curvature is - (open universe)')
...
# Setting parameters and solving Einstein Eqns
...
# klist = np.array([1e-5, 1e-4])
nulist = tc.sqrt((tc.tensor(klist) / Kscale)**2 + 1)
...
x_in = eta_in * Kscale
x_0 = eta_0 * Kscale
binlist = np.where(nulist<1000, 1000, np.ceil(nulist))
binlist = np.array(binlist, dtype='int32')
